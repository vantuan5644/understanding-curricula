...
parser.add_argument("--ordering", default="curr", type=str,
                    help="which test case to use. supports: standard, curriculum, anti and random")
...
def main():
    ...
    if 'cscores-orig-order.npz' in args.order_dir:
        ...
        if not os.path.isfile(temp_path):
            print(
                'Downloading the data cifar10-cscores-orig-order.npz and cifar100-cscores-orig-order.npz to folder orders')
    ...
    bs = args.batchsize
    N = len(order)
    myiterations = (N // bs + 1) * args.epochs  # total iterations
    ...
    if args.ordering == "standard":
        ...
        for epoch in range(args.epochs):
            tr_loss, tr_acc1, iterations = train(train_loader, model, criterion, optimizer, scheduler, epoch,
                                                 iterations)
        ...
        while step < myiterations:
            ...
            if startIter_next <= N:
                ...
                print("%s iter data between %s and %s w/ Pacing %s and LEARNING RATE %s " % (
                    step, startIter, startIter_next, args.pacing_f, optimizer.param_groups[0]["lr"]))
                train_loader = torch.utils.data.DataLoader(
                    Subset(tr_set, list(order[startIter:max(startIter_next, 256)])), \
